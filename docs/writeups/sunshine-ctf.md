...
# Sunshine_CTF_writeup 

This line marks the beginning of the writeup by BITSkrieg for the Sunshine CTF which was hosted by Hack@UCF in affiliation with BSides Orlando 2025.
We participated in this prestigious tournament and placed 12th overall in the competition.
Below will be a few writeups to be submitted by BITSkrieg:

## Writeup No.1:(Pegasus) Can you hEAR me?

This one was pretty easy to do. The question stated:
...
You just had to run the peg file using _runpeg_ directly and terminal just spits out the flag:
...
## Writeup No.2 :(Pegasus) Access Code

    This one a bit tricky to understand. The question stated:
...
I had initially got a dbg prompt where I had to type forgot and got a hint hash encoded 64 bit string but it didn't take me anyway from there.
So I had to start from scratch and first dissassemble the code and got this:
...
If we observe closely there is a hash function named ghimli
which actually tells a lot about the latter part of challenge.
 The process is implemented in @gimli_hash and has the follwing:

        1. call @gimli_hash_init
        2. call @gimli_absorb
        3. call @gimli_hash_final  
After calling the above three functions it supposedly takes hash of some data we dont know and is trying to compare it with the user inputs I am entering.
So after a lot of time and trial and error I tried using the debugger once agian to find A1 parameter of @gimli_absorb_byte.(Earlier it was missing out the bytes of @gimli_absorb_byte so had to do this step)
It printed out something like this:
    ...
    '''bash
        ...
        @gimli_absorb_byte:
                ...
                04F2.0000: XOR     A3, A1
                04F4.0000: STB     [A2],A3            
After joining and reconstruction of the flag by using things like: 
    ```python

    >>> bytes.fromhex("73756e")
    b'sun'   
We get the flag sun{th3_fun_p4r7_15_nEAR}
    ```.

## Writeup No.3:(Pegasus)Login Screen
This one was easier than the erlier Access code challenge and it didn't have the flag scattered all roung the memory and encoded or something.
The question was:
...
For the first time in my life I liked a challenge since it was fun to solve it!
I started by debugging and dissassembling the code:
        ...
        @main:
                ...
                0295.0000: STW     [FP],FP
                0297.0000: BRR     @main
It was clear from here that stack overflow is happening at **@read_line** .We can use buffer overflow to change PC to point to @win function when 0292.0000: POP {S0, FP, PC-DPC} is running to extract the flag out!

Using the basic pwn tools and a bit of coding to print the flag:
        ...
        p.interactive()
      ```
And just like that the flag was extracted: sun{th1s_i5_ju57_7h3_t1p_0f_7h3_spEAR}    

## Writeup No.4 :(i95) Miami

Even this was an easy one. The question stated:
...
Decompiled the code in Ghidra and we get:
        int vuln()
        {
...
Stack overflow using **gets**. We can override the memory location 0x1337C0DE by writing a simple python code and pwn tools:
...
## Writeup No.5 :(i95)Jupiter 

This was a fun one. Had to first learn how to shift memory and then write the code on shifting of memory.
First I debugged the code using Ghidra:
...
The code wants the flag's meomery at **0x1337C0DE** and only then will it print the flag. Now we have to find the secret key and get the memory location of the key.
...
The key is located at 0x404010 with inital value of 0x0BADC0DE
So the problem remains that instead of **0x1337C0DE** we need to place **0x404012** which is 2 bytes higher than the key.
Then I was trying to figure out the format string placement on the stack memory.
After trying out a few exploitations I reached printf exploitation:
        $ ./jupiter
        ...
        At and after the fifth parameter the format string is representing the parameter.So to write **0x1337C0DE** we can do the following:
...
Now all we need to do is write a python code using pwn tools to get the key at **0x404010** and  write **0x1337C0DE** with **0x404012** to get the flag!
...
## Writeup No.6 :(Re) Space Race

No this one was a medium level chal but a lot of reading and disection of the code is required.
...
There's a lot to uncover from here on. It kinda looks like a game like astroid busters and might need a bit help with the controls part.
We can see a lot of things like:
    1. 2 bytes of op code
    2. 1 byte of payload length
    3. variable length of payload
So now we need to assign opcodes with necessary functions to make it playable and retrieve the flag!
...
Now just map the opcodes to keyboard input(Used a bit of gpt!):
...
Then initiate the game by pressing **r** and ude Left/Right to avoid obstacles.After completing it we get the flag: sun{r3d_r0v3r_c0m3_0v3r}

## Writeup No.7 :(i95) Daytona

This challenge was a good one. The question was:
...
Start by decompiling the code in Ghidra:
...
We can see that it leeks adress from somewhere. "maybe the stack adress?" is what I thought and then examined the stack frame and in the function  *vul* in *sp* we see that:

1. sp+0x00 to sp+0x10: saved x29 and x30
2. sp+0x10: gets buffer is here

Hence we cannot override he return adress from *vuln*. BUT we can from *main*

"sp+0x50 to sp+0x60: saved x29 and x30 for main"

So I could may be override the sp+0x58 to somewhere else in the stack.
So I put the shellcode generated by pwntools there to get shell:
```python
from pwn import *
...
# gdb.attach(p)
# pause()
# gets buffer is at sp+0x10(very imp)
p.sendline(b"A" * 0x48 + p64(sp+0x60) + asm(shellcode))
p.interactive()

This will print out the flag: sun{ARM64_shEl1c0de_!s_pr3ttY_n3a7_dOnT_y0u_thInk?}
